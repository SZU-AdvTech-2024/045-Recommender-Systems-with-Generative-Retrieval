...
from utils import ensure_dir, set_color, delete_file
import os
...
class Trainer(object):
    ...
    def __init__(self, args, model, data_num):
        ...
        # steps before reaching the maximum learning rate
        ...
        self.ckpt_dir = os.path.join(args.ckpt_dir, f'{args.token_type}')
        if args.token_type == 'sid':
            dataset_name = args.dataset + '_' + args.tokenizer_plm
        elif args.token_type == 'pretrained':
            dataset_name = args.dataset
        self.ckpt_dir = os.path.join(self.ckpt_dir, dataset_name)
        token_size = f'K={args.num_emb_list[0]}_D={len(args.num_emb_list)}'
        self.ckpt_dir = os.path.join(self.ckpt_dir, token_size)
    ...
    def _get_scheduler(self):
        if self.lr_scheduler_type.lower() == "linear":
            ...
            # before num_warmup_steps, lr increase linearly
            # after, constant
    ...
    def fit(self, data):
        ...
        for epoch_idx in range(self.epochs):
            ...
            if (epoch_idx + 1) % self.eval_step == 0:
                ...
                # # reverse collision, construct max-heap
                ...
                #     self.newest_save_queue.append(now_save)
                #     # now collision is smaller than heap top, replace it
